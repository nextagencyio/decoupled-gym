...
import trainersData from '@/data/mock/trainers.json'
import membershipsData from '@/data/mock/memberships.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetTrainers') || query.includes('nodeTrainers')) {
      return loadMockData('trainers.json')
    }

    if (query.includes('GetMemberships') || query.includes('nodeMemberships')) {
      return loadMockData('memberships.json')
    }
