...
import { GET_MEMBERSHIP_BY_PATH } from '@/lib/queries'
import { DrupalMembership } from '@/lib/types'
...
interface MembershipByPathData {
  route: {
    entity: DrupalMembership
  } | null
}
...
async function getMembership(path: string): Promise<DrupalMembership | null> {
  try {
    ...
    const { data } = await apolloClient.query<MembershipByPathData>({
      query: GET_MEMBERSHIP_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/memberships/${slug.join('/')}`
  const item = await getMembership(path)

  if (!item) {
    return { title: 'Membership Not Found | Gym' }
  }
