...
export default function CTASection({ homepageContent }: CTASectionProps) {
  const title = (homepageContent as any)?.ctaTitle || 'Ready to Transform Your Body?'
  const description = (homepageContent as any)?.ctaDescription?.processed || ''
  const primaryLabel = (homepageContent as any)?.ctaPrimary || 'Start Your Free Trial'
  const secondaryLabel = (homepageContent as any)?.ctaSecondary || 'View Memberships'
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-8">
        ...
        {description ? (
          ...
          <p className="text-primary-100 text-lg mb-10 max-w-2xl mx-auto">
            Join thousands of members who have already started their fitness journey. First week is on us.
          </p>
        )}
