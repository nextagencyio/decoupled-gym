...
import { GET_TRAINER_BY_PATH } from '@/lib/queries'
import { DrupalTrainer } from '@/lib/types'
...
interface TrainerByPathData {
  route: {
    entity: DrupalTrainer
  } | null
}

async function getTrainer(path: string): Promise<DrupalTrainer | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<TrainerByPathData>({
      query: GET_TRAINER_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/trainers/${slug.join('/')}`
  const item = await getTrainer(path)

  if (!item) {
    return { title: 'Trainer Not Found | Gym' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).experienceYears && (
                    <div>
                      <dt className="text-sm text-gray-500">Years of Experience</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).experienceYears}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
